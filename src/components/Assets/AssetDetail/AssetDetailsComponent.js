...
class AssetDetails extends React.Component {
    constructor() {
        ...
        this.state = {
            ...
            inspectionPage:0,inspectionRowsPerPage:20,
            workorderPage:0,workorderRowsPerPage:20,
            currentTimeZone:'',
            meterHours:null, formError: {}, errorMessage: {},	tostMsg:{}
        }
        ...
        setTimeout(function () {
            ...
            setTimeout(() => {
                ...
                console.log("in comoponent did mount",_.get(this, ['props', 'inspectionList'], []))
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + this.state.inspectionPageSize + "/" + this.state.inspectionPageIndex
    ...
    handleTabSelect(key, assetDetails) {
        ...
        if (key == 'inspections') {
            ...
            $('#pageLoading').show();
            var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + this.state.workorderPageSize + "/" + this.state.workorderPageIndex
    ...
    handleSearchOnKeyDown = (e, searchString) => {
        ...
        if (e.key === 'Enter') {
            if (searchString) {
                this.setState(
                    { inspectionPageIndex: 1 ,inspectionPage:0},
                    () => {
                        isSearchEnable = true;
                        var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(searchString) + "/" +this.state.currentTimeZone +"/"+this.state.inspectionRowsPerPage + "/" + this.state.inspectionPageIndex
                        $('#pageLoading').show();
                ...
                this.setState(
                    ...
                    () => {
                        isSearchEnable = true
                        var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + this.state.inspectionRowsPerPage + "/" + this.state.inspectionPageIndex
                         $('#pageLoading').show();
                         this.props.inspectionListAction(urlParameters,this.state.inspectionPageIndex);
                    });
            }
    ...
    handleSearchLoadMore = (e, searchString) => {
        ...
        if (searchString) {
            this.setState(
                { inspectionPageIndex: this.state.inspectionPageIndex + 1 },
                () => {
                    isSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(searchString) + "/" +this.state.currentTimeZone +"/"+ this.state.inspectionPageSize + "/" + this.state.inspectionPageIndex
    ...
    handleWorkOrderSearchOnKeyDown = (e, searchString) => {
        ...
        if (e.key === 'Enter') {
            if (searchString) {
                this.setState(
                    { workorderPageIndex: 1 ,workorderPage:0},
                    () => {
                        isWorkOrderSearchEnable = true;
                        var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(searchString) + "/" + this.state.workorderRowsPerPage + "/" + this.state.workorderPageIndex
                        $('#pageLoading').show();
                ...
                this.setState(
                    ...
                    () => {
                        isWorkOrderSearchEnable = true;
                        var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + this.state.workorderRowsPerPage + "/" + this.state.workorderPageIndex
                         $('#pageLoading').show();
                         this.props.workOrderListAction(urlParameters,this.state.workorderPageIndex);
                    });
            }
    ...
    handleSearchWorkOrderLoadMore = (e, searchString) => {
        ...
        if (searchString) {
            this.setState(
                { workorderPageIndex: this.state.workorderPageIndex + 1 },
                () => {
                    isWorkOrderSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(searchString) + "/" + this.state.workorderPageSize + "/" + this.state.workorderPageIndex
    ...
    handleInspectionlistChangePage = (event, newPage) => {
        ...
        if (insepectionSearchString) {
            this.setState(
                ...
                () => {
                    isSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(insepectionSearchString) + "/" +this.state.currentTimeZone +"/"+ this.state.inspectionRowsPerPage + "/" + this.state.inspectionPageIndex
                    if (this.state.isInspectionDataNotFound) {
                    } else {
                        $('#pageLoading').show();
                        this.props.inspectionSearchList(urlParameters, this.state.inspectionPageIndex);
            ...
            if (this.props.inspectionList.length != totalInspectionListCnt) {
                ...
                if (insepectionSearchString) {
                    this.setState(
                        ...
                        () => {
                            isSearchEnable = true;
                            var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(insepectionSearchString) + "/" +this.state.currentTimeZone +"/"+ rowPerPage + "/" + this.state.inspectionPageIndex
                            $('#pageLoading').show();
                            this.props.inspectionSearchList(urlParameters, this.state.inspectionPageIndex);
                    ...
                    this.setState(
                        ...
                        () => {
                            isSearchEnable = true
                            var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + rowPerPage + "/" + this.state.inspectionPageIndex
                             $('#pageLoading').show();
                             this.props.inspectionListAction(urlParameters,this.state.inspectionPageIndex);
                        });
                }
    ...
    handleWorkOrderlistChangePage = (event, newPage) => {
        ...
        if (workOrderSearchString) {
            ...
            this.setState(
                ...
                () => {
                    isWorkOrderSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(workOrderSearchString) + "/" + this.state.workorderPageSize + "/" + this.state.workorderPageIndex
                    if (this.state.isWorkOrderDataNotFound) {
                    } else {
                        $('#pageLoading').show();
                        this.props.workOrderSearchList(urlParameters, this.state.workorderPageIndex);
    ...
    handleWorkOrderlistChangeRowsPerPage = (rowPerPage) => {
        ...
        if (workOrderSearchString) {
            this.setState(
                ...
                () => {
                    isWorkOrderSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" + encodeURI(workOrderSearchString) + "/" + rowPerPage + "/" + this.state.workorderPageIndex
            ...
            this.setState(
                ...
                () => {
                    isWorkOrderSearchEnable = true;
                    var urlParameters = this.state.loginData.uuid + "/" + _.get(this, ['props', 'assetDetails', 'asset_id'], '') + "/" +rowPerPage + "/" + this.state.workorderPageIndex
                    console.log("urlParameters=============",urlParameters);
                    $('#pageLoading').show();
                     this.props.workOrderListAction(urlParameters,this.state.workorderPageIndex);
                });
    ...
    render() {
        ...
        console.log("tost msg ----------",this.state.tostMsg)
        const { formError, errorMessage } = this.state; 
                ...
                <Grid className="inspection-title bottom-lines">
                    ...
                    <Grid className="inspection-breadcrum">
                        <ul className="bread-crum">
                            ...
                            <li> > </li>
                            <li><a href="javascript:void(0)">{_.get(assetDetails, ['internal_asset_id'], '')}</a></li>
                        </ul>
                    </Grid>
                </Grid>
                    ...
                    <Grid className="row">
                        <Grid className="col-sm-12 col-xs-12 col-md-12 col-xl-5 col-xs-12 assettable">
                            <Grid className="assets-info-container ">
                                <Grid className="row">
                                    <form>
                                        <Grid className="assets-info-title"><h4>Asset Info</h4></Grid>
                                            ...
                                            <Grid className="row" >
                                                        ...
                                                        <Grid className="col-md-6">
                                                            <Grid className="assets-info-devider mb-0">
                                                                <TextField
                                                                    ...
                                                                    margin="normal"
                                                                    fullWidth
                                                                    value={_.get(assetDetails, ['status_name'], '')}
                                                                    id="status"
                                                                    label="status"
                                                                    name="status"
                                                                    disabled="true"
                                                      ...
                                                      <Grid className="assets-info-devider mb-0">
                                                       ...
                                                       <div className="asset-status">Status</div>
                                                           ...
                                                           <FormControlLabel
                                                               ...
                                                               checked={status== enums.assetStatus[0].id?true:false}
                                                           ...
                                                           />
                                                            <FormControlLabel
                                                               ...
                                                               checked={status== enums.assetStatus[1].id?true:false}
                                                ...
                                                <Grid className="col-md-4 order-1">
                                                    ...
                                                    {_.get(assetDetails, ['asset_photo'], '')?
                                                     ...
                                                     <img alt="Sensaii" src={_.get(assetDetails, ['asset_photo'], '')} style={{ "width": "140px", "height": "140px" ,"border": "1px solid #212128" }} className="MuiAvatar-img" />
                                                     </Grid>
                                                    :
                                                    <Grid className="user-profile">
                                                    <img alt="Sensaii" src={noImageAvailable} style={{ "width": "140px", "height": "140px","border": "1px solid #212128" }} className="MuiAvatar-img" />
                                                    </Grid>
                                                     } 
                                            ...
                                            <Grid className="col-md-12">
                                                            <Grid className="assets-info-devider mb-0">
                                                                <TextField
                                                                    ...
                                                                    margin="normal"
                                                                    fullWidth
                                                                    value={_.get(assetDetails, ['internal_asset_id'], '')}
                                                                    id="internalAssetId"
                                                                    label="#"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['asset_type'], '')}
                                                            id="objectType"
                                                            label="Object Type"
                                                            name="objectType"
                                                            disabled="true"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['product_name'], '')}
                                                            id="product"
                                                            label="Product"
                                                            name="product"
                                                            disabled="true"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['model_name'], '')}
                                                            id="model"
                                                            label="Model"
                                                            name="model"
                                                            disabled="true"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        ...
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['asset_serial_number'], '')}
                                                            id="serialnumber"
                                                            label="Serial Number"
                                                            name="serialNumber"
                                                            disabled="true"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['model_year'], '')}
                                                            id="modelYear"
                                                            label="Model Year"
                                                            name="modelYear"
                                                            disabled="true"
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['site_name'], '')}
                                                ...
                                                <Grid className="col-md-12">
                                                    <Grid className="assets-info-devider mb-0">
                                                        <TextField
                                                            ...
                                                            margin="normal"
                                                            fullWidth
                                                            value={_.get(assetDetails, ['current_stage'], '')}
                                                            id="currentStage"
                                                            label="Current stage"
                                                            name="currentStage"
                                                            disabled="true"
                                                ...
                                                <Box className="assets-buttons-part " style={{"margin": "30px 165px"}}>
                                                    <Button variant="contained" color="primary" className="assets-bottons txt-normal" style={{ "fontSize": "13px" }} onClick={this.UpdateMeterHour}>Save</Button>
                                                </Box>
                                            </Grid>
                                        </Grid>
                                    </form>
                                ...
                                <Paper>
                                    <AppBar position="static" color="inherit">
                                        <Tabs id="controlled-tab-example"  activeKey={this.state.selecttab} onSelect={k => this.handleTabSelect(k, assetDetails)}>
                                            <Tab eventKey="inspections" title="Inspection">
                                                {/* <InspectionList title="Inspections"  inspectionList={_.get(assetDetails,['inspections'],[])}/> */}
                                            </Tab>
                                            <Tab eventKey="workOrder" title="Work Order" workOrderList={this.state.workOrderList}
                                    ...
                                    {this.state.selecttab == 'inspections' ?
                                        <InspectionList inspectionList={_.get(this, ['props', 'inspectionList'], [])}
                                            ...
                                            />
                                        :
                                        <WorkOrderList workOrderList={workOrderList} assetId={this.props.asset_id}
...
export default connect(mapState, actionCreators)(AssetDetails);
...
import pdf from "../../Content/images/pdf.svg";
import CSV from "../../Content/images/CSV.svg";
...
class Reports extends React.Component {
    ...
    componentDidMount() {
        ...
        var timeClear = setInterval(() => {
            ...
            if (_.get(reducer, ['historyReportList', 'list'], []).length) {
                ...
                // console.log("inProgressReportList ", inProgressReportList);


                if (inProgressReportList.length) {

                    console.log("checking for report status every 3 sec", list);
                    var requestData = {
    ...
    handlefromDateChange = date => {
        ...
        this.setState({ fromDate: date, formError, errorMessage })
        ...
        if (this.state.report == enums.reportType[4].type) {
            ...
            if ( !formError['toDate']) {
                if (toDate > after3MonthDate) {
                    formError['toDate'] = true;
                    errorMessage['toDate'] = 'Please select the date range of 3 months only';
                } else {
                    delete formError['toDate']
                    delete errorMessage['toDate']
                }
    ...
    handleGenerateReport = () => {
        ...
        if (formvalid) {
            if (this.state.report == enums.reportType[0].type) {//report 1
                $('#pageLoading').show();
                var urlParameters = this.state.loginData.uuid + "/" + moment(this.state.fromDate).format('YYYY-MM-DD') + "/" + moment(this.state.toDate).format('YYYY-MM-DD')
                this.props.generateReportListAction(urlParameters);
                this.setState({ isGenerateReport: true, isLatestHourReadingReport: false })
            }
            else if (this.state.report == enums.reportType[1].type) {//report 2
                $('#pageLoading').show();
                var urlParameters = this.state.loginData.uuid + "/" + moment(this.state.fromDate).format('YYYY-MM-DD') + "/" + moment(this.state.toDate).format('YYYY-MM-DD');
                this.props.generateReport2ListAction(urlParameters);
                this.setState({ isGenerateReport: true, isLatestHourReadingReport: false })
            }
            else if (this.state.report == enums.reportType[2].type) {//report 3
                $('#pageLoading').show();
                var urlParameters = this.state.loginData.uuid + "/" + moment(this.state.fromDate).format('YYYY-MM-DD') + "/" + moment(this.state.toDate).format('YYYY-MM-DD');
                this.props.generateReport3ListAction(urlParameters);
                this.setState({ isGenerateReport: true, isLatestHourReadingReport: false })
            }
            else if (this.state.report == enums.reportType[3].type) {//report 4
                $('#pageLoading').show();
    ...
    handleOnChnage = (e,value) => {
        ...
        delete errorMessage['assetId']
        if(value){
            this.setState({ assetId: value.internal_asset_id, formError, errorMessage })
        }else{
            this.setState({ assetId: '', formError, errorMessage })
        }
    ...
    render() {
        ...
        const createReport1Data = (yard, approvedinspection, totalinspection) => {
            return { yard, approvedinspection, totalinspection };
        }
        const createReport2Data = (yard, equipment_type, approvedinspection, totalinspection) => {
            return { yard, equipment_type, approvedinspection, totalinspection };
        }
        const createReport3Data = (yard, internal_asset_id, asset_name, begin_hours, end_hours, approvedinspection, totalinspection) => {
            return { yard, internal_asset_id, asset_name, begin_hours, end_hours, approvedinspection, totalinspection };
        }

        const createReport4Data = (internal_asset_id, asset_name, meter_hours, inspection_date) => {
            return { internal_asset_id, asset_name, meter_hours, inspection_date };
        }
        const createReport5Data = (report_id, asset_name, start_date, end_date, status, statusID, download_link) => {
            return { report_id, asset_name, start_date, end_date, status, statusID, download_link };
        }
        ...
        if (reportlist.length > 0) {
            ...
            reportlist.map((value, key) => {
                ...
                if (this.state.report == enums.reportType[0].type) {
                    ...
                    var result = createReport4Data(value.internal_asset_id, value.asset_name, value.current_meter_hours, value.latest_inspection_date)
                    rows.push(result);
                }
                else if (this.state.report == enums.reportType[4].type) {
        ...
        const filterOptions = createFilterOptions({
            matchFrom: "start",
            stringify: (option) => option.internal_asset_id.trim(),
            trim: true
          });
        return (
            <div >
                                                                            ...
                                                                            <Grid className="assets-info-devider ss-multi-select">
                                                                                <FormControl fullWidth variant="outlined" className={classes.formControl}>
                                                                                    ...
                                                                                    <Select
                                                                                        ...
                                                                                        onChange={(e) => { this.handleReportchnage(e) }}
                                                                                        error={formError.report}
                                                                                        helperText={errorMessage.report}
                                                                                    >
                                                                                ...
                                                                                <Grid className="assets-info-devider ss-multi-select">
                                                                                    <Autocomplete
                                                                                       filterOptions={filterOptions}
                                                                                        ...
                                                                                        getOptionLabel={(option) => option.internal_asset_id}
                                                                                        name="assetId"
                                                                                        // value={this.state.assetId}
                                                                                        onChange={(e, newValue) => { this.handleOnChnage(e, newValue) }}
                                                                                        inputValue={this.state.vendorInputValue}
                                                                                        onInputChange={(event, newInputValue) => {
                                                                                            this.handleInputOnChnage(event, newInputValue)
                                                                                        }}  
                                                                                        noOptionsText="No asset found"                                                          
                                                                                        // disableClearable={_.isEmpty(this.state.assetId) ? true : false}
                                                                                       
                                                                                        renderInput={(params) => <TextField
                                                                                            {...params}
                                                                                            error={formError.assetId}
                                                                                            variant="outlined"
                                                                                            margin="normal"
                                                                                            fullWidth
                                                                                            placeholder="Select Asset No"
                                                                                            name="assetId"
                                                                                            helperText={errorMessage.assetId}
                                                                                        />}
                                                                                    />
                                                                                </Grid>
                                                                            </div>
                                                                    ...
                                                                    {!this.state.isLatestHourReadingReport && this.state.report ?
                                                                        <Grid className="col-md-3">
                                                                            <Grid className="assets-info-devider">
                                                                                ...
                                                                                {/* {formError.fromDate ? <div className={classes.validation}>{errorMessage.fromDate}</div>
                                                                                    : ''} */}
                                                                            </Grid>
                                                                        </Grid>
                                                                        : ''}
                                                                    {!this.state.isLatestHourReadingReport  && this.state.report?
                                                                        <Grid className="col-md-3">
                                                                            <Grid className="assets-info-devider">
                                                                                ...
                                                                                {/* {formError.toDate ? <div className={classes.validation}>{errorMessage.toDate}</div>
                                                                                    : ''} */}
                                                                            </Grid>
                                                                        </Grid>
                                                                        : ''}
                                                                        {this.state.report?
                                                                          ...
                                                                          <Grid className="assets-info-devider">
                                                                              ...
                                                                              <Button variant="contained" color="primary" className="assets-bottons txt-normal" style={{ "fontSize": "13px", "marginTop": "27px", "width": "fit-content" }} onClick={this.handleGenerateReport}>Generate</Button>
                                                                          </Grid>
                                                                      </Grid>
                                                                        :''}
                                    ...
                                    <Grid item xs={12}>
                                        <Paper className={classes.paper}>
                                           ...
                                           <img src={xls} className="dwndl_btn_icon"></img>
                                                ...
                                                {this.state.report != enums.reportType[4].type ?
                                                    <ExcelFile element={<img src={xls} className="dwndl_btn_icon"></img>} filename="ReportExcelFile">
                                                ...
                                                {/* <CSVLink data={rows} filename="ReportExcelFile.csv">
                                           <img src={CSV} className="dwndl_btn_icon"></img>
                                       </CSVLink>
                                   */}
                                                {/* <PDFDownloadLink
                                       document={<PdfDocument data={rows} />}
                                       fileName="RportPdf.pdf"
                                       
                                   >
                                       <img src={pdf} className="dwndl_btn_icon"></img>
                                            ...
                                            <Grid item xs={12}>
                                                        ...
                                                        <TableRow>
                                                            {tableHeader().map((headCell, key) => {
                                                                return (<TableCell key={key}
                                                                    id={headCell.id}
                                                                    align={headCell.id == 'download_link' ? 'center' : 'left'}
                                                                ...
                                                                (rows.map((tableRow, key) => {
                                                                    return (
                                                                        <TableRow key={key}>
                                                                            ...
                                                                            <TableCell className={classes.tableCell}>{tableRow.inspection_date ?
                                                                                momenttimezone.utc(tableRow.inspection_date).tz("America/Los_Angeles").format('MM-DD-YYYY hh:mm:ss a')
                                                                                : '-'}</TableCell>
                                                                        </TableRow>)
                                                                }))
                                                                : ''}
                                                                ...
                                                                (rows.slice(this.state.page * this.state.rowsPerPage, this.state.page * this.state.rowsPerPage + this.state.rowsPerPage).map((tableRow, key) => {
                                                                    return (
                                                                        <TableRow key={key}>
                                                                            ...
                                                                            <TableCell className={classes.tableCell}>{tableRow.asset_name ? tableRow.asset_name : '-'}</TableCell>
                                                                            <TableCell className={classes.tableCell}>{tableRow.start_date != undefined ?
                                                                                ...
                                                                                : '-'}</TableCell>
                                                                            <TableCell className={classes.tableCell}>{tableRow.end_date != undefined ?
                                                                            ...
                                                                            {tableRow.statusID == enums.reportStatus[0].id?
                                                                                <CircularProgress />
                                                                                 :(tableRow.statusID == enums.reportStatus[1].id ?
                                                                                        <a href={tableRow.download_link}><ArrowDownwardIcon color="primary"/></a>:
                                                                                    '-')}</TableCell>
                                                                        </TableRow>)
                                                ...
                                                {_.isEmpty(rows) ?
                                                    <div className="report-nodata">No data found</div>
                                                    : ''}
                                                ...
                                                {this.state.report == enums.reportType[4].type ?
                                                    (_.isEmpty(rows) ? '' :
                                                        <TablePagination
                                                            ...
                                                            component="div"
                                                            count={this.props.reportsReducer.historyReportList.listsize}
                ...
                {_.isEmpty(this.state.toastMsg)?'':<SnackbarAlert tostMsg={_.get(this, ['props', 'reportsReducer', 'tostMsg'], {})} />}
            </div>
        )
    }
}

			...
			reader.onload = (e) => {
				...
				this.setState({ data: data, cols: make_cols(ws['!ref']) }, () => {
					...
					JSON.parse(jsonData).map((value,key)=>{
						...
						if(key == 0){//prepare all object key
							...
							_.forOwn(value, function(value1, key1) {
							 ...
							 _.forOwn(value, function(value2, key2) {
								...
								if(Object.keys(value).indexOf(key2)<position[0]){
									...
									inspectionObject["inspection_form"].push(
										{"name":value2,"value":""}
									)
									inspectionFormAttributes.push(
										{"name":value2,"value":""})
										colObj.push(value2);
								}
							 });
					...
					requestData.inspection.map((value,key)=>{
							...
							>
								...
								{companyList.length > 0 ?
									(companyList.map((value, key) => {
										return (
											<option value={value.company_id} key={key}>{value.company_name}</option>
										)
									}))
									: ''}

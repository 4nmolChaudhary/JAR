...
export default function assetListAction(urlParameters) {
    ...
    return dispatch => {
        ...
        validateAssetIdService(urlParameters).then(response => {
            $('#pageLoading').hide();
    ...
    function request(isValidAsset,tostMsg) { return { type: userConstants.VALIDATE_ASSET_REQUEST,isValidAsset,tostMsg} }
    function success(isValidAsset,tostMsg) { return { type: userConstants.VALIDATE_ASSET_SUCCESS, isValidAsset ,tostMsg} }
    function failure(error,tostMsg) { return { type: userConstants.VALIDATE_ASSET_FAILURE, error ,tostMsg} }
}
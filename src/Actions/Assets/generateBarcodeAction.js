...
export default function generateBarcodeAction(requestData) {
	...
	return dispatch => {
		...
		generateBarcode(requestData).then(response => {
	...
	function request(generateBarcode,tostMsg) { return { type: userConstants.GENERATE_BARCODE_REQUEST, generateBarcode ,tostMsg } }
	function success(generateBarcode,tostMsg) { return { type: userConstants.GENERATE_BARCODE_SUCCESS, generateBarcode ,tostMsg} }
	function failure(error,tostMsg) { return { type: userConstants.GENERATE_BARCODE_FAILURE, error ,tostMsg} }
}
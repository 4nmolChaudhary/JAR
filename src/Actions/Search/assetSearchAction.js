...
export default function assetSearchListAction(urlParamerter,pageIndex) {
    ...
    return dispatch => {
        ...
        assetSearchList(urlParamerter).then(response => {
            $('#pageLoading').hide();
    ...
    function request(assetList,tostMsg) { return { type: userConstants.ASSETLIST_SEARCH_REQUEST,assetList,tostMsg} }
    function success(assetList,searchString,tostMsg) { return { type: userConstants.ASSETLIST_SEARCH_SUCCESS,pageIndex, assetList ,searchString,tostMsg} }
    function failure(error,tostMsg) { return { type: userConstants.ASSETLIST_SEARCH_FAILURE, error ,tostMsg } }
}
...
export default function searchInUserListAction(urlParamerter,pageIndex) {
    ...
    return dispatch => {
        ...
        searchInUserList(urlParamerter).then(response => {
            $('#pageLoading').hide();
    ...
    function request(userList,tostMsg) { return { type: userConstants.USER_LIST_SEARCH_REQUEST,userList , tostMsg} }
    function success(userList,searchString,tostMsg) { return { type: userConstants.USER_LIST_SEARCH_SUCCESS,pageIndex, userList ,searchString , tostMsg} }
    function failure(error,tostMsg) { return { type: userConstants.USER_LIST_SEARCH_FAILURE, error , tostMsg} }
}
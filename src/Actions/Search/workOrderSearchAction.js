...
export default function workOrderSearchListAction(urlParamerter,pageIndex) {
    ...
    return dispatch => {
        ...
        workOrderSearchList(urlParamerter).then(response => {
            $('#pageLoading').hide();
    ...
    function request(workOrderList,tostMsg) { return { type: userConstants.WORKORDERLIST_SEARCH_REQUEST,workOrderList,tostMsg} }
    function success(workOrderList,searchString,tostMsg) { return { type: userConstants.WORKORDERLIST_SEARCH_SUCCESS,pageIndex, workOrderList ,searchString,tostMsg} }
    function failure(error,tostMsg) { return { type: userConstants.WORKORDERLIST_SEARCH_FAILURE, error,tostMsg } }
}   
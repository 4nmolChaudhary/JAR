...
export default function workOrderUpdateAction(requestData) {
    ...
    return dispatch => {
        ...
        workOrderUpdate(requestData).then(response => {
            $('#pageLoading').hide();
    ...
    function request(workOrderUpdate,tostMsg) { return { type: userConstants.WORKORDERUPDATE_REQUEST,workOrderUpdate,tostMsg} }
    function success(workOrderUpdate,tostMsg) { return { type: userConstants.WORKORDERUPDATE_SUCCESS, workOrderUpdate,tostMsg } }
    function failure(error,tostMsg) { return { type: userConstants.WORKORDERCREATE_FAILURE, error ,tostMsg} }
}
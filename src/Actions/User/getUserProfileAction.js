...
export default function getUserProfileAction(urlprameters) {
    ...
    return dispatch => {
        ...
        getUserProfile(urlprameters).then(response => {
            $('#pageLoading').hide();
    ...
    function request(userDetail,tostMsg) { return { type: userConstants.USER_DETAIL_PROFILE_REQUEST,userDetail ,tostMsg} }
    function success(userDetail,tostMsg) { return { type: userConstants.USER_DETAIL_PROFILE_SUCCESS, userDetail ,tostMsg } }
    function failure(error,tostMsg) { return { type: userConstants.USER_DETAIL_PROFILE_FAILURE, error ,tostMsg} }
}
...
export default function getUserRolesAction(urlPrameters) {
    return dispatch => {
        dispatch(request({}));
        getUserRoles(urlPrameters).then(response => {
            $('#pageLoading').hide();
    ...
    function request(userRoles) { return { type: userConstants.GET_USER_ROLES_REQUEST,userRoles} }
    function success(userRoles) { return { type: userConstants.GET_USER_ROLES_SUCCESS, userRoles } }

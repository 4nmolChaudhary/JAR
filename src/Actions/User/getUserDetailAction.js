...
export default function getUserDetailAction(urlprameters) {
    ...
    return dispatch => {
        ...
        getuserDetail(urlprameters).then(response => {
            $('#pageLoading').hide();
    ...
    function request(userDetail,tostMsg) { return { type: userConstants.USER_DETAIL_REQUEST,userDetail,tostMsg} }
    function success(userDetail,tostMsg) { return { type: userConstants.USER_DETAIL_SUCCESS, userDetail ,tostMsg} }
    function failure(error,tostMsg) { return { type: userConstants.USER_DETAIL_FAILURE, error ,tostMsg} }
}